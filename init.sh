...
setup_fedora() {
    ...
    # Prompt for Wireguard installation
    read -p "Would you like to install Wireguard? (y/n): " install_wireguard
    ...
    read -p "Would you like to install Kmonad? (y/n): " install_kmonad
    if [[ "$install_kmonad" =~ ^[Yy]$ ]]; then
        use_kmonad=true
        TOTAL_STEPS=$((TOTAL_STEPS + 1))
        echo "[✓] Kmonad will be installed."
    else
        use_kmonad=false
        echo "[✓] Skipping Kmonad installation."
    fi
    echo ""
    ...
    if [[ "$change_git" =~ ^[Yy]$ ]]; then
        change_git_remote=true
        TOTAL_STEPS=$((TOTAL_STEPS + 1))
        echo "[✓] Git remote will be changed"
    else
        change_git_remote=false
        echo "[✓] Git remote wiill not be changed"
    fi
    echo ""
    ...
    # Install Wireguard (conditional)
    if [ "$use_wireguard" = true ]; then
        STEP=$((STEP + 1))
        echo "[$STEP/$TOTAL_STEPS] Installing Wireguard..."
        sudo dnf -y install wireguard-tools
        sudo dnf -y install python3-pip python3-gobject gtk3
    ...
    if [ "$change_git_remote" = true ]; then
      STEP=$((STEP + 1))
